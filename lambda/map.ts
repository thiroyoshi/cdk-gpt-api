...
export const choicesHandler: APIGatewayProxyHandler = async (event) => {
  ...
  let tmpMAX = process.env.PLACES_RESULT_MAX || 20;
  ...
  const bordingTimeDate = new Date().setHours(
    bordingTimeHour,
    bordingTimeMinute,
    0,
    0
  ...
  const axiosConfig = {
    headers: {
      ...
      "X-Goog-FieldMask":
        "places.displayName,places.formattedAddress,places.websiteUri,places.types",
    },
    timeout: 20000,
  };
...
export const directionHandler: APIGatewayProxyHandler = async (event) => {
  ...
  const url =
    DIRRECTIONS_API_ENDPOINT +
    "?origin=" +
    origin +
    "&destination=" +
    address +
    "&travelMode=DRIVING&languageCode=" +
    languageCode +
    "&key=" +
    GOOGLE_API_KEY;
...
export const directionDescriptionHandler: APIGatewayProxyHandler = async (
  event
) => {
  const DIRRECTIONS_API_ENDPOINT = process.env.DIRRECTIONS_API_ENDPOINT || "";
  if (DIRRECTIONS_API_ENDPOINT == "") {
  ...
  try {
    ...
    console.error("Error with Google API or OpenAI API:", error);
    return createResponse(500, {
      status: "error",
      message: error,
    });
